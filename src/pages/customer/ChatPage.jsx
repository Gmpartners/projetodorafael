  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (chatId && chatId !== 'general-chat') {
        try {
          console.log('📤 Calling apiService.sendMessage with:', {
            chatId: chatId,
            messageData: messageData
          });
          
          const sendResponse = await apiService.sendMessage(chatId, messageData);
          console.log('✅ Message sent successfully, response:', sendResponse);
          ...
          setMessages(prev => prev.map(msg => 
            msg.id.startsWith('temp-') ? { ...msg, id: `error-${Date.now()}`, content: msg.content + ' (Failed to send)' } : msg
          ));
          
          // Show detailed error
          alert(`Failed to send message: ${apiError.response?.data?.error || apiError.message}`);
        }
      } else {
