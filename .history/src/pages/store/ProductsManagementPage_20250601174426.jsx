...
const ProductsManagementPage = () => {
  ...
  // 🔧 FUNÇÃO CORRIGIDA: Validação robusta
  const validateForm = () => {
    const newErrors = {};
  ...
  const debugProductData = (product) => {
    console.log('🔍 DEBUG: Dados do produto para edição');
    console.log('Produto original:', product);
    console.log('CustomSteps:', product.customSteps);
    
    if (product.customSteps) {
      product.customSteps.forEach((step, index) => {
        console.log(`Etapa ${index}:`, {
          name: step.name,
          scheduledFor: step.scheduledFor,
          timeValue: step.timeValue,
  ...
  // 🔧 FUNÇÃO CORRIGIDA: handleEditProduct com parsing robusto
  const handleEditProduct = (product) => {
    debugProductData(product); // Debug dos dados originais
    
    setEditingProduct(product);
    
    // 🔧 CONVERSÃO ROBUSTA das etapas
    const formattedSteps = (product.customSteps || []).map((step, index) => {
      let timeValue = 1;
      let timeUnit = 'hours';
      
      console.log(`🔄 Processando etapa ${index}:`, step);
      
      // Verificar se já está no formato correto (timeValue/timeUnit separados)
      if (step.timeValue && step.timeUnit) {
        timeValue = parseInt(step.timeValue) || 1;
        timeUnit = step.timeUnit || 'hours';
        console.log(`✅ Formato direto: ${timeValue} ${timeUnit}`);
      } 
      // Caso contrário, tentar parsear scheduledFor
      else if (step.scheduledFor && typeof step.scheduledFor === 'string') {
        const parts = step.scheduledFor.trim().split(' ');
        console.log(`🔧 Parseando scheduledFor: "${step.scheduledFor}" -> partes:`, parts);
        ...
        if (parts.length >= 2) {
          ...
          if (!isNaN(parsedValue) && parsedValue > 0) {
            ...
            timeUnit = parts[1];
            console.log(`✅ Parse bem-sucedido: ${timeValue} ${timeUnit}`);
          } else {
            console.log(`❌ Parse falhou, usando padrão: ${timeValue} ${timeUnit}`);
          }
        } else {
          console.log(`❌ Formato inválido, usando padrão: ${timeValue} ${timeUnit}`);
        }
      } else {
        console.log(`❌ scheduledFor inválido, usando padrão: ${timeValue} ${timeUnit}`);
      }
      ...
      console.log(`✅ Etapa ${index} formatada:`, formattedStep);
      return formattedStep;
    });
    
    console.log('📋 Todas as etapas formatadas:', formattedSteps);
  ...
  // 🔧 FUNÇÃO CORRIGIDA: updateCustomStep com debug e validação
  const updateCustomStep = (index, field, value) => {
    console.log(`🔧 updateCustomStep: index=${index}, field=${field}, value=${value}, type=${typeof value}`);
    
    setNewProduct(prev => {
      const newSteps = prev.customSteps.map((step, i) => {
        if (i === index) {
          const updatedStep = { ...step, [field]: value };
          console.log(`📝 Etapa ${i} atualizada:`, updatedStep);
          return updatedStep;
        }
        return step;
      });
      
      console.log('📋 Novo estado customSteps:', newSteps);
      return {
        ...prev,
  ...
  return (
    <MainLayout userType="store" pageTitle="Gestão de Produtos">
      <div className="space-y-4 sm:space-y-6 w-full max-w-full overflow-hidden">
        ...
        {/* Modal de Adicionar Produto - igual ao original */}
        <CustomModal 
          ...
          maxWidth="1000px"
        >
          {/* ... conteúdo do modal de adicionar produto igual ao original ... */}
        </CustomModal>

        {/* Modal de Visualizar Produto - igual ao original */}
        <CustomModal 
          isOpen={isViewModalOpen} 
          onClose={() => setIsViewModalOpen(false)}
          maxWidth="1000px"
        >
          {/* ... conteúdo do modal de visualizar produto igual ao original ... */}
        </CustomModal>

        {/* Modal de Editar Produto - COM CORREÇÕES */}
        <CustomModal 
            ...
            <div className="flex-1 overflow-hidden p-6">
              <div className="h-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto max-h-[60vh]">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex space-x-2">
                                      {/* 🔧 INPUT CORRIGIDO com melhor handling */}
                                      <Input
                                        type="number"
                                        min="1"
                                        placeholder="1"
                                        ...
                                        onChange={(e) => {
                                          ...
                                          // Permitir string vazia temporariamente, mas converter para número na atualização
                                          const numericValue = inputValue === '' ? '' : parseInt(inputValue) || 1;
                                          console.log(`Input onChange: "${inputValue}" -> ${numericValue}`);
                                          updateCustomStep(index, 'timeValue', numericValue);
                                        }}
                                        onBlur={(e) => {
                                          // Garantir que sempre há um valor válido ao sair do campo
                  ...
                  {/* Tab de informações básicas permanece igual */}
                  {activeTab === 'basic' && (
                    /* ... conteúdo da tab básica igual ao original ... */
                  )}
                </div>
              </div>
            </div>
